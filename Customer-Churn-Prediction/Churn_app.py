...
st.markdown(""" 
    <style>
        ...
        .nochurn{background-color:#e8f5e9; color:#2e7d32}
    </style>                
...
st.title("Predict Customer Churn With Machine Learning")
st.markdown("---")
if model is not None:
    ...
    with col1:
        ...
        partner = st.selectbox("Partener",["Yes","No"])
    ...
    with col2:
        ...
        tensure = st.slider("Tensure (Months)",0,72,12)
        contract = st.selectbox("Contract",["Month-to-month","One year","Two year"])
        paperlress_billing = st.selectbox("Paperless Billing",["Yes","No"])
        payment_method = st.selectbox("Payment Method",["Electronic check","Mailed check","Bank Transfer (automatic)","Credit card (automatic)"])
        monthly_charges = st.number_input("Monthly Charges($)",0.0,150.0,50.0,5.0)
        total_charges = st.number_input("Total Charges($)",0.0,10000.0,monthly_charges * tensure,50.0)
    st.markdown("---")
    if st.button("Predict"):
        input_data = {
            'gender':[gender],"SeniorCitizen":[senior_citizen],'Partner':[partner],
            'Dependents':[dependents],'tenure':[tensure],'PhoneService':[phone_service],
            "MultipleLines":[multiple_lines],'InternetService':[internet_service],
            "OnlineSecurity":[online_security],'OnlineBackup':[online_backup],
            'DeviceProtection':[device_protection],'TechSupport':[tech_support],
            'StreamingTV':[streaming_tv],'StreamingMovies':[streaming_movies],
            'Contract':[contract],'PaperlessBilling':[paperlress_billing],
        ...
        if prediction==1:
            st.warning("**At Risk:**Offer retention inactives, upgrae to longer contract, provide better support")
        else:
            st.warning("**Low Risk:**Continue excellent service, send surveys, offer loyalty benefits")
    with st.sidebar:
        st.header("Model Info")
        st.info("""
        **XGBoost Classifier**
        - Test Accuracy : ~77%
        - Training : 7,063 customers
